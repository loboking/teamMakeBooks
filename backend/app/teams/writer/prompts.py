...
def build_rep_polish_prompt(draft: str, feedback: str) -> str:
    """반복 패턴만 집중 교체하는 프롬프트. 본문 전체 재작성 금지."""
    return (
        "[반복 패턴 정제 — 교체만 수행]\n\n"
        f"[검수자 피드백]\n{feedback.strip()}\n\n"
        "[정제 지시]\n"
        "- 위 피드백에서 지적된 반복 패턴만 교체. 사건·플롯·대사·시스템 메시지는 100% 보존.\n"
        "- '이준' → 문맥에 맞게 '그는', '그가', '그의' 또는 주어 생략(~했다로 시작).\n"
        "- 대사 안의 '이준 씨', '이준아' 등은 절대 변경 금지.\n"
        "- 동일 행동 동사(바라보았다, 확인했다 등) → 응시했다, 시선을 주었다, 눈여겨보았다 등으로 다양화.\n"
        "- 캐릭터 등급/직업 묘사(F급 짐꾼 등) 중복 → 첫 번째만 남기고 나머지는 행동 묘사로 교체.\n"
        "- 연속 같은 주어 시작 → 대명사 또는 주어 생략으로 문장 구조 변경.\n"
        "- 분량은 원본과 ±10% 이내.\n"
        "- 완성된 본문만 그대로 출력. 해설·메타코멘트·[원문]/[수정] 마크업 절대 금지.\n\n"
        f"[입력 본문]\n{draft.strip()}\n\n"
        "[정제된 본문]"
    )
