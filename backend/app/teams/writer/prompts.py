...
def _get_genre_tone_block(persona, ctx) -> str:
    ...
    if "슬로우" in genre or "무역" in genre or "라이프" in genre:
        return (
            ...
            "- 서술에서 다음 단어 사용 금지 (한계: 미세하게 3회, 미묘하게 2회):\n"
            "  존재론적, 에너지 흐름, 시스템적, 이질적, 물리적 법칙, 물리적 흐름, 차원의, 근원적\n"
            "  불일치, 포착, 측정, 계산했다, 분석했다, 인식했다, 깨달았다, 직감했다 — 행동으로 보여줘라\n"
            "- 키워드로 캐릭터를 설명하지 마라. 행동으로 보여라:\n"
            "  ❌ '하린은 그 무게를 계산했다' / '하린은 이것이 가짜라는 것을 깨달았다'\n"
            "  ✅ '빵이 50전인데 동전 두 개는 100전. 거스름이 100전이다' / '풀빛이 손끝에서 떨렸다. 다른 풀들은 그렇지 않았다'\n"
            "- 특별함을 설명할 때 추상 용어 대신 **구체적 감각**을 써라:\n"
            "  ❌ '존재론적 무게를 지니고 있었다'\n"
            "  ✅ '손에 쥔 돌이 따뜻했다. 주변 돌들은 차가웠는데, 이것만 체온처럼'\n\n"
            "### 룰 8. 패턴 금지 — 같은 구조 반복 금지\n"
            "- ❌ 관찰→발견→주머니에 넣기 패턴을 1화에 2회 이상 반복 금지.\n"
            "- 첫 번째 발견은 자세히 묘사, 두 번째는 간단히 처리하거나 다른 행동(대화/거래 시도 등)을 섞어라.\n\n"
            "### 룰 9. 게임UI·시스템창 금지\n"
            "- 본문에 대괄호[...]로 시스템 알림·분류창·숫자 표기 금지.\n"
            "- ❌ '[물질 분류: 비정형 자원]'\n"
            "- ❌ '[현실 조작 지수: 0.001%]'\n"
            "- 시스템창이 필요하면 캐릭터의 생각이나 대사로 표현:\n"
            "  ✅ '이건 보통 돌이 아니다. 무게가 다르고, 온도가 다르다. 팔면 될까?'\n\n"
            "### 룰 10. 감정 표현 강제\n"
            "- 빵을 먹었으면 맛에 대한 반응을 행동으로: 찡그림, 천천히 씹기, 다시 한 입\n"
            "- 낯선 환경이면 걸음이 느려지거나, 손을 쥐었다 폈다, 주변을 두 번 확인\n"
            "- 감정 0개의 기계적 묘사는 AI 글이다. 반응은 작아도 꼭 있어야 한다."
        )
